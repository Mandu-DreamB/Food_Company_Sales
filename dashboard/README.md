...
- **접근 제한 없음**: 포트폴리오로 공개하기 위해 접근 토큰 게이트(`APP_ACCESS_TOKEN`)를 없앴다.
  이 엔드포인트는 요청 1건에 LLM을 2번 부르므로, 공개 배포 상태에서 트래픽이 몰리면 OpenAI
  비용이 그대로 나간다는 점을 인지하고 사용할 것 (요청량 제한은 아직 코드에 없음).
